...
class MiCanvas extends JPanel implements MouseListener
{
	...
	*/
	public void generar(int max)
	{
		...
		Vertice p;
		Random a=new Random();
		Vertice v1 = new Vertice(100,100);
                Vertice v2 = new Vertice(200,200);
                Vertice v3 = new Vertice(100,400);
                Vertice v4 = new Vertice(400,400);	
                vertices.add(v1);	
			d.insertar(v1);
                        vertices.add(v2);	
			d.insertar(v2);
			...
			for(int i = 0; i < d.aristas.size(); i++)
			{
				...
				pantalla.drawLine(((Arista)d.aristas.get(i)).getDestino().getX(),
							 ...
							 -((Arista)d.aristas.get(i)).getGemelo().getDestino().getY());
                                    ...
                                    System.out.println("arista "+i+"en y:" + ((Arista) d.aristas.get(i)).getDestino().getY());
                                    System.out.println("arista "+i+"en genela x:" + ((Arista) d.aristas.get(i)).getGemelo().getDestino().getX());
                                    System.out.println("arista "+i+"en gemela y:" + ((Arista) d.aristas.get(i)).getGemelo().getDestino().getY());
				}
